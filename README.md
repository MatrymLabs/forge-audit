...
| `license` | a LICENSE file (SPDX tag or text signature) or a `pyproject` declaration, plus any provenance artifacts (third-party notices / attribution), plus a **per-file `SPDX-License-Identifier` scan**, a **supply-chain scan of installed dependency licenses**, and **validation of a committed CycloneDX/SPDX SBOM, cross-checked against what is actually installed** | a recognized license is present *and* no source file declares a conflicting license *and* no installed dependency carries strong (GPL/AGPL) copyleft *and* any committed SBOM both parses *and* still reflects the installed deps (else a watchlist gap: reuse rights unclear, a **copyleft file in a permissive repo** = possible contamination, a **strong-copyleft dependency** = distribution obligation, a **malformed SBOM** = false assurance, or a **stale SBOM** that omits much of what is installed = drifted evidence). A valid SBOM enriches the evidence with its format + component count; a repo shipping no SBOM is not penalized, and freshness is only judged when the target's env is present |
