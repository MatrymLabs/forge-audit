...
| `license` | a LICENSE file (SPDX tag or text signature) or a `pyproject` declaration, plus any provenance artifacts (third-party notices / attribution), plus a **per-file `SPDX-License-Identifier` scan**, a **supply-chain scan of installed dependency licenses**, and **validation of a committed CycloneDX/SPDX SBOM** | a recognized license is present *and* no source file declares a conflicting license *and* no installed dependency carries strong (GPL/AGPL) copyleft *and* any committed SBOM actually parses (else a watchlist gap: reuse rights unclear, a **copyleft file in a permissive repo** = possible contamination, a **strong-copyleft dependency** = distribution obligation, or a **malformed SBOM** = false assurance). A valid SBOM enriches the evidence with its format + component count; a repo shipping no SBOM is not penalized |
